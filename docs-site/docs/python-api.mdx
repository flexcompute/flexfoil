...
polar.plot()  # 4-panel matplotlib figure: CL-α, CD-α, CL-CD, CM-α
...
The browser app reads from the same SQLite database as the Python API. Runs
solved from the web UI appear in `flexfoil.runs()`, and vice versa.
...
flowchart LR
    ...
    subgraph browser ["Browser (localhost:8420)"]
        spa["React SPA"]
        wasm["WASM solver"]
        rest["REST API"]
...
The [`examples/`](https://github.com/flexfoil/flexfoil/tree/main/packages/flexfoil-python/examples) directory contains runnable scripts:
