...
C------ Debug output: transition detection
        CALL DBGTRAN(IS, IBL, XOCTR(IS), TRFORC)
      ENDIF
C
      TRAN = .FALSE.
C
...
C      
      ...
      ENDIF
C
C---- Debug output for Newton system coefficients
      CALL DBGSETBL(IS, IBL, IV, NSYS, VA, VB, VM, VDEL, IVX)
C
